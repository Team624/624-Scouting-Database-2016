<?php
class FmsModel {
  public static $EVENT_CODE = 'TXDA';
  public static $trimFields = [
    'QS','assist','auto','trussCatch','teleop'
  ];
  public function getRankings() {
    $text = file_get_contents('http://www2.usfirst.org/2015comp/events/' . self::$EVENT_CODE . '/rankings.html');
    $doc = new DOMDocument();
    @$doc->loadHTML($text);
 ...
 //   echo var_dump($ret);
  ...
  public function getResults() {
    $text = file_get_contents('http://www2.usfirst.org/2015comp/events/' . self::$EVENT_CODE . '/matchresults.html');
    $doc = new DOMDocument();
    @$doc->loadHTML($text);
    $rows = $doc->getElementsByTagName('table')->item(2)->getElementsByTagName('tr');
    $results = [];
